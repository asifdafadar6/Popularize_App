...
const BusinessProfileDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const profileResponse = await axios.get(`https://popularizenode.apdux.tech/api/viewprofilebyid/${businessId}`);
        ...
        if (offersResponse.data.success) {
          const businessOffers = offersResponse.data.data.filter(
            offer => offer.businessId?._id === businessId
          );
  ...
  const isOwnProfile = userId === businessId;
          ...
          <>
            <View style={styles.header}>
              <View style={styles.icon}>
                <Image source={{ uri: userData.profileImage }} style={styles.profileImages} />
              </View>
...
const styles = StyleSheet.create({
  ...
  instaname: { fontSize: 14, color: 'gray', padding: 2 },
  businessInfo: {
    fontSize: 14,
    color: 'white',
    marginBottom: 15,
